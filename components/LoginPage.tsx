    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center p-4 font-sans">
      ...
      <div className="flex flex-col md:flex-row gap-6 w-full max-w-4xl justify-center items-stretch mb-16 animate-scale-in">
        ...
        <button 
          onClick={onGuest}
          disabled={isLoading}
          className="flex-1 bg-white rounded-[2.5rem] p-8 shadow-[0_4px_40px_-12px_rgba(0,0,0,0.05)] border border-slate-100 hover:shadow-[0_20px_40px_-12px_rgba(0,0,0,0.1)] hover:border-slate-200 hover:-translate-y-1 transition-all duration-300 group text-center flex flex-col items-center justify-between"
        >
          <div className="flex flex-col items-center">
            <div className="w-20 h-20 bg-[#F1F5F9] rounded-[1.5rem] flex items-center justify-center mb-6 group-hover:bg-slate-200 transition-colors">
                <User className="w-10 h-10 text-slate-400 group-hover:text-slate-600 transition-colors stroke-[1.5]" />
            </div>
            
            <h2 className="text-2xl font-bold text-slate-800 mb-2">Guest Mode</h2>
            <p className="text-slate-500 leading-relaxed max-w-[240px] mx-auto text-sm font-medium mb-6">
                Test drive the app immediately. Data is stored locally in your browser.
            </p>
          </div>

          <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 max-w-[260px] w-full mx-auto flex gap-3 text-left">
              <AlertTriangle className="w-5 h-5 text-amber-500 flex-shrink-0 mt-0.5" />
              <p className="text-[10px] text-amber-800 font-bold leading-tight">
                  Note: Guest data is not synced and will be erased if you clear your browser cache.
              </p>
          </div>
        </button>
        ...
        <button 
          onClick={onLogin}
          disabled={isLoading}
          className="flex-1 bg-white rounded-[2.5rem] p-8 shadow-[0_4px_40px_-12px_rgba(0,0,0,0.05)] border border-slate-100 hover:shadow-[0_20px_40px_-12px_rgba(0,0,0,0.1)] hover:border-slate-200 hover:-translate-y-1 transition-all duration-300 group text-center flex flex-col items-center relative overflow-hidden justify-between"
        >
           {isLoading && (
          ...
          <div className="flex flex-col items-center">
            <div className="w-20 h-20 bg-[#F1F5F9] rounded-[1.5rem] flex items-center justify-center mb-6 group-hover:bg-white group-hover:shadow-md transition-all">
                {/* Google G Logo SVG */}
           ...
           <div className="p-3 w-full"></div> {/* Spacer to match height */}
        </button>
      </div>
